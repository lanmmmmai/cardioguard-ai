...
async def delete_user(
    ...
    # Kiểm tra người dùng tồn tại
    check_user = await database.fetch_one("SELECT id, role FROM users WHERE id = :user_id::uuid", {"user_id": user_id})
    if not check_user:
        raise HTTPException(status_code=404, detail="Không tìm thấy người dùng")
    ...
    await database.execute("UPDATE users SET status = 'inactive' WHERE id = :user_id::uuid", {"user_id": user_id})
