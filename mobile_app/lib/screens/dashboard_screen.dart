...
class _DashboardScreenState extends State<DashboardScreen> with TickerProviderStateMixin {
  ...
  void _onWebSocketEvent(Map<String, dynamic> event) {
    ...
    if (type == 'health_metrics' && patientId == _selectedPatientId) {
      ...
      if (alertData['is_resolved'] == false) {
        setState(() {
          _activeBannerMessage = '${alertData['full_name'] ?? 'Bệnh nhân'}: ${alertData['message']}';
        });
        _triggerBannerFlash();
      }
    }
  ...
  void _cancelSos() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Đã hủy yêu cầu SOS.'), backgroundColor: Colors.orange),
    );
  }

  Future<void> _sendSosAlert() async {
    final alertProvider = Provider.of<AlertProvider>(context, listen: false);
    final success = await alertProvider.triggerSosAlert('YÊU CẦU SOS KHẨN CẤP TỪ THIẾT BỊ DI ĐỘNG');
    if (mounted) {
      if (success) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Gửi SOS thất bại. Kiểm tra kết nối mạng!'), backgroundColor: Colors.red),
        );
      }
    }
  }
        ...
        child: Column(
          children: [
            ...
            if (_activeBannerMessage != null)
              Container(
                color: _isBannerFlash ? const Color(0xFFFF0055) : const Color(0xFF9E002E),
                ...
                child: Row(
                  children: [
                    ...
                    IconButton(
                      ...
                      onPressed: () => setState(() => _activeBannerMessage = null),
                    ),
                  ],
                ),
              ),
              ...
              child: Row(
                ...
                children: [
                  Column(
                    ...
                    children: [
                      Text(
                        role == 'patient' ? 'Chỉ số sức khỏe' : 'Giám Sát Trung Tâm',
                        style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: textColor),
                      ),
                      const SizedBox(height: 4),
                      if (role != 'patient' && patientProvider.patients.isNotEmpty)
                        Row(
                          children: [
                            Text('Bệnh nhân: ', style: TextStyle(color: textMuted, fontSize: 13)),
                            DropdownButton<String>(
                              value: _selectedPatientId,
                              dropdownColor: cardBg,
                              style: const TextStyle(color: Color(0xFFFF3366), fontWeight: FontWeight.bold, fontSize: 14),
                  ...
                  IconButton(
                    ...
                    icon: Icon(
                      ...
                      color: isDark ? const Color(0xFFFFB606) : const Color(0xFF475467),
                    ),
                  ...
                  children: [
                    ...
                    if (role == 'admin') ...[
                      GridView.count(
                        ...
                        children: [
                          _buildSummaryCard('Bệnh nhân', '${patientProvider.patients.length}', LucideIcons.users, Colors.blue, cardBg, textColor, textMuted, borderColor),
                          _buildSummaryCard('Cảnh báo hoạt động', '${alertProvider.activeAlertCount}', LucideIcons.bell, Colors.red, cardBg, textColor, textMuted, borderColor),
                        ],
                      ),
                    ...
                    GridView.count(
                      ...
                      children: [
                        _buildMetricCard('NHỊP TIM', '$hrVal', 'BPM', LucideIcons.heart, const Color(0xFFFF3366), isAbnormal && hrVal > 100, cardBg, textColor, textMuted, borderColor),
                        _buildMetricCard('NỒNG ĐỘ OXY (SPO2)', '$spo2Val', '%', LucideIcons.droplet, const Color(0xFF00F2FE), isAbnormal && spo2Val < 94, cardBg, textColor, textMuted, borderColor),
                        _buildMetricCard('HUYẾT ÁP (BP)', '$sysBp/$diaBp', 'mmHg', LucideIcons.activity, const Color(0xFF39FF14), isAbnormal && (sysBp > 135 || sysBp < 95), cardBg, textColor, textMuted, borderColor),
                      ],
                    ),
                    const SizedBox(height: 16),
                      ...
                      child: Column(
                        ...
                        children: [
                          Row(
                            mainAxisAlignment: MainAxisAlignment.spaceBetween,
                            children: [
                              Text('ĐIỆN TÂM ĐỒ (ECG)', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: textMuted)),
                              Container(
                    ...
                    if (role == 'patient') ...[
                        ...
                        child: Column(
                          children: [
                            ...
                            if (_isSosCounting) ...[
                              Column(
                                children: [
                                  Text(
                                    ...
                                    style: const TextStyle(color: Color(0xFFFF0055), fontWeight: FontWeight.bold, fontSize: 16),
                                  ),
                                  const SizedBox(height: 12),
                                  ElevatedButton(
                                    onPressed: _cancelSos,
                                    style: ElevatedButton.styleFrom(backgroundColor: Colors.grey),
                                    child: const Text('Hủy bỏ yêu cầu', style: TextStyle(color: Colors.white)),
                                  ),
                                ],
                              ),
                            ] else ...[
                              ...
                              Text('Nhấn giữ nút để gửi', style: TextStyle(color: textMuted, fontSize: 11)),
                            ],
                          ],
                        ),
                      ),
                      ...
                      Container(
                        ...
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            const Text('THIẾT BỊ MÔ PHỎNG LÂM SÀNG', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                            const SizedBox(height: 6),
                            Text('Mô phỏng trạng thái sinh học bất thường để kiểm tra hệ thống báo động.', style: TextStyle(color: textMuted, fontSize: 12)),
                            const SizedBox(height: 16),
                            Row(
                              children: [
                                Expanded(
                                  child: OutlinedButton(
                                    onPressed: _selectedPatientId == null ? null : () => patientProvider.triggerSimulation(_selectedPatientId!, false),
                                    style: OutlinedButton.styleFrom(
                                      side: const BorderSide(color: Color(0xFF39FF14)),
                                      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
                                    ),
                                    child: const Text('Bình thường', style: TextStyle(color: Color(0xFF39FF14))),
                                  ),
                                ),
                                const SizedBox(width: 12),
                                Expanded(
                                  child: ElevatedButton(
                                    onPressed: _selectedPatientId == null ? null : () => patientProvider.triggerSimulation(_selectedPatientId!, true),
                                    style: ElevatedButton.styleFrom(
                                      backgroundColor: const Color(0xFFFF3366),
                                      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
                                    ),
                                    child: const Text('Bất thường', style: TextStyle(color: Colors.white)),
                                  ),
                                ),
                              ],
                            ),
  ...
  Widget _buildSummaryCard(String title, String value, IconData icon, Color color, Color cardBg, Color textColor, Color textMuted, Color borderColor) {
    return Container(
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(title, style: TextStyle(fontSize: 10, color: textMuted, fontWeight: FontWeight.bold)),
                const SizedBox(height: 2),
                Text(value, style: TextStyle(fontSize: 18, color: textColor, fontWeight: FontWeight.bold)),
              ],
            ),
          ),
        ],
            ...
            child: Row(
              ...
              children: [
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        Text(unit, style: TextStyle(fontSize: 10, color: textMuted)),
                      ],
                    )
                  ],
                ),
                Container(
                  width: 28,
                  height: 28,
                  decoration: BoxDecoration(color: glowColor, borderRadius: BorderRadius.circular(8)),
